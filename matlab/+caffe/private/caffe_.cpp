...
#define MEX_ARGS int nlhs, mxArray **plhs, int nrhs,  mxArray **prhs

using namespace caffe;  // NOLINT(build/namespaces)

// Do CHECK and throw a Mex error if check fails
inline void mxCHECK(bool expr,  char* msg) {
  if (!expr) {
    mexErrMsgTxt(msg);
  }
}
inline void mxERROR( char* msg) { mexErrMsgTxt(msg); }

// Check if a file exists and can be opened
void mxCHECK_FILE_EXIST( char* file) {
  std::ifstream f(file);
  if (!f.good()) {
    f.close();
...
static void mx_mat_to_blob( mxArray* mx_mat, Blob<float>* blob,
...
static mxArray* blob_to_mx_mat( Blob<float>* blob,
    WhichMemory data_or_diff) {
...
static mxArray* int_vec_to_mx_vec( vector<int>& int_vec) {
...
static mxArray* str_vec_to_mx_strcell( vector<std::string>& str_vec) {
...
template <typename T>
static T* handle_to_ptr( mxArray* mx_handle) {
...
static mxArray* create_handle_vec(int ptr_num) {
   int handle_field_num = 2;
   char* handle_fields[handle_field_num] = { "ptr", "init_key" };
...
template <typename T>
static void setup_handle( T* ptr, int index, mxArray* mx_handle_vec) {
...
template <typename T>
static mxArray* ptr_to_handle( T* ptr) {
...
template <typename T>
static mxArray* ptr_vec_to_handle_vec( vector<shared_ptr<T> >& ptr_vec) {
...
static void delete_solver(MEX_ARGS) {
  ...
  solvers_.erase(std::remove_if(solvers_.begin(), solvers_.end(),
      [solver] ( shared_ptr< Solver<float> > &solverPtr) {
...
static void solver_get_attr(MEX_ARGS) {
  mxCHECK(nrhs == 1 && mxIsStruct(prhs[0]),
      "Usage: caffe_('solver_get_attr', hSolver)");
  Solver<float>* solver = handle_to_ptr<Solver<float> >(prhs[0]);
   int solver_attr_num = 2;
   char* solver_attrs[solver_attr_num] = { "hNet_net", "hNet_test_nets" };
...
static void delete_net(MEX_ARGS) {
  ...
  nets_.erase(std::remove_if(nets_.begin(), nets_.end(),
      [net] ( shared_ptr< Net<float> > &netPtr) {
...
static void net_get_attr(MEX_ARGS) {
  ...
  Net<float>* net = handle_to_ptr<Net<float> >(prhs[0]);
   int net_attr_num = 6;
   char* net_attrs[net_attr_num] = { "hLayer_layers", "hBlob_blobs",
...
static void layer_get_attr(MEX_ARGS) {
  ...
  Layer<float>* layer = handle_to_ptr<Layer<float> >(prhs[0]);
   int layer_attr_num = 1;
   char* layer_attrs[layer_attr_num] = { "hBlob_blobs" };
...
static void blob_reshape(MEX_ARGS) {
  mxCHECK(nrhs == 2 && mxIsStruct(prhs[0]) && mxIsDouble(prhs[1]),
      "Usage: caffe_('blob_reshape', hBlob, new_shape)");
  Blob<float>* blob = handle_to_ptr<Blob<float> >(prhs[0]);
   mxArray* mx_shape = prhs[1];
  double* shape_mem_mtr = mxGetPr(mx_shape);
   int num_axes = mxGetNumberOfElements(mx_shape);
