...
void hdf5_save_nd_dataset<float>(
  ...
  const float* data;
  if (write_diff) {
	data = const_cast<Blob<float>*>(&blob)->cpu_diff();
  } else {
	data = const_cast<Blob<float>*>(&blob)->cpu_data();
  }
...
void hdf5_save_nd_dataset<double>(
  ...
  const double* data;
  if (write_diff) {
	data = const_cast<Blob<double>*>(&blob)->cpu_diff();
  } else {
	data = const_cast<Blob<double>*>(&blob)->cpu_data();
  }
